...
def batch_preprocessing(clip, spatial_transform): 
    batch_clip = []
    for batch_index, per_clip in enumerate(clip):
        # Applying spatial transformations
        if spatial_transform is not None:
            spatial_transform.randomize_parameters()
            # Before applying spatial transformation you need to convert your frame into PIL Image format (its not the best way, but works)
            per_clip = [spatial_transform(Image.fromarray(np.uint8(img)).convert('RGB')) for img in per_clip]
        # Rearange shapes to fit model input
        per_clip = torch.stack(per_clip, 0).permute(1, 0, 2, 3)
        # print (">>>", clip.shape)
        batch_clip.append( per_clip )
    batch_clip = torch.stack((batch_clip), 0)
    print (">>>", batch_clip.shape)
    return batch_clip

def predict(clip, model, spatial_transform, classes, batch_flag=True):
    # Set mode eval mode
    model.eval()
    # do some preprocessing steps
    if batch_flag:
        clip = batch_preprocessing(clip, spatial_transform)
    else:
    ...
    for sc, cl in zip( list(score.cpu().detach().numpy()), list(class_prediction.cpu().detach().numpy())  ):
...
while True:
        ...
        if len(full_clip)==opt.sample_duration:
            batch_full_clip.append( full_clip )
            full_clip = []
        if len(batch_full_clip)==128:
            try:
                probs, class_names = predict( batch_full_clip, model, spatial_transform, classes=2, batch_flag=True ) 
